...
import { User, Mail, Phone, Music, Award, LogOut, UserPlus, CheckCircle2, Loader2, AlertCircle } from 'lucide-react';
...
import TeacherActivationCard from '@/components/activation/TeacherActivationCard';
import LinkTeacherCard from '@/components/activation/LinkTeacherCard';
...
export default function StudentSettings() {
  ...
  const { data: assignedTeacher } = useQuery({
    queryKey: ['assignedTeacher', user?.assigned_teacher_id],
    queryFn: async () => {
      if (!user?.assigned_teacher_id) return null;
      const teachers = await base44.entities.User.filter({ id: user.assigned_teacher_id });
      return teachers[0] || null;
    },
    enabled: !!user?.assigned_teacher_id,
  });

  // Get all teachers with agent codes (for matching)
  const { data: teacherAgentInfos } = useQuery({
    queryKey: ['teacherAgentInfos'],
    queryFn: () => base44.entities.AgentInfo.filter({ role: 'teacher' }),
    enabled: !user?.assigned_teacher_id,
  });
  ...
  // Connect to teacher by agent code
  const handleConnectToTeacher = async () => {
    if (!agentCode || agentCode.length < 4) {
      setConnectError('Please enter a valid 4-character code');
      return;
    }
    ...
    try {
      const codeUpper = agentCode.toUpperCase().trim();
      
      // Method 1: Search all admin users (teachers) for matching agentpass
      const allTeachers = await base44.entities.User.filter({ role: 'admin' });
      const matchedTeacher = allTeachers.find(
        teacher => teacher.agentpass && teacher.agentpass.toUpperCase() === codeUpper
      );

      if (matchedTeacher) {
        // Found teacher - assign student to this teacher
        await base44.auth.updateMe({ assigned_teacher_id: matchedTeacher.id });
      ...
      // Method 2: Check AgentInfo entity for teacher records
      const allAgentInfos = await base44.entities.AgentInfo.list();
      const matchingTeacherInfo = allAgentInfos.find(
        info => info.agentpass && info.agentpass.toUpperCase() === codeUpper && info.role === 'teacher'
      );

      if (matchingTeacherInfo) {
        // The student_id in teacher's AgentInfo is the teacher's user ID
        const teacherId = matchingTeacherInfo.student_id;
        
        await base44.auth.updateMe({ assigned_teacher_id: teacherId });
      ...
      // No match found
      setConnectError('Invalid code. Please check with your teacher and try again.');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="max-w-4xl mx-auto p-6 lg:p-8 space-y-8">
        ...
        {/* Teacher Activation - Basic users can become teachers (only if not already a teacher) */}
        {user.role === 'user' && user.app_role !== 'teacher' && (
          <TeacherActivationCard />
        )}

        {/* Link to Teacher */}
        <LinkTeacherCard currentTeacher={assignedTeacher} />
